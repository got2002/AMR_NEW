	...
	<section class="pt-2 pb-4">
		<div class="flex justify-between items-center mb-10">
			...
			<div class="flex gap-2">
				<UIBackButton @click="$router.push(localePath({ name: 'project' }))">{{ $t("button.back") }}</UIBackButton>

				<!-- <nuxt-link :to="localePath({ name: 'project' })" class="px-6 py-3 text-center bg-gray-500 rounded shadow-sm text-white hover:bg-gray-600">{{ $t("button.back") }}</nuxt-link> -->
				<nuxt-link :to="localePath('/project/' + data._id + '/edit')" class="px-6 py-3 bg-yellow-500 hover:bg-yellow-600 rounded shadow-sm text-theme-white">{{ $t("button.edit") }}</nuxt-link>
			</div>
		</div>
		...
		<div v-if="!loading" class="flex items-start gap-4">
			<div id="info" class="flex flex-col w-1/2 bg-white shadow-md p-4">
				...
				<div class="col-span-1 bg-theme-black-50 bg-opacity-50 p-4 gap-3 mt-5">
					...
					<div class="flex">
						<div class="text-sm font-bold">{{ $t("project.view_page.account") }} :</div>
						<div class="px-2 text-sm">
							{{ dropdowns.accounts.find((acc) => acc.value === data.desAccountNumber)?.accountName || $t("undefined") }}
						</div>
					</div>
				</div>
