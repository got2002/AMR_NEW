<template>
	<div>
		<div class="relative">
			...
			<div :id="'mapEdit-' + form._id" :style="{ width: width + widthUnit, height: height + heightUnit }" class="z-10 border border-carbon-sorf-dark"></div>
		</div>
		<div class="w-full grid grid-cols-1 xl:grid-cols-1 2xl:grid-cols-1 mt-4 overflow-x-auto">
			<table class="items-center w-full space-y-6 border border-theme-black-300">
				<thead>
					<tr>
						<template v-for="(item) in table.head">
							<ApiHeadSlotWhite :text="item.name" :key="item.name" :align="item.align" />
						</template>
					</tr>
				</thead>
				<tbody class="bg-white border border-theme-black-300">
					<template v-for="(data, index) in form.location.features">
						<ProjectTableLocation :feature="data" :locationIndex="index" @updateFeature="updateFeature" @deleteFeature="deleteFeature" @flyTo="flyTo" :selected="select" :ref="'TableLocation' + index" :key="'TableLocation' + index" />
						<tr :key="'adress-thai' + index">
							<td colspan="2">
								<div class="flex items-center text-sm">
									<label class="w-48 px-3 py-2 text-center border-2 bg-gray-200">{{ $t("project.create_page.address.subtitle_lang.th") }}</label>
									<input v-model="form.address[index]" type="text" class="w-full px-3 py-2 text-center border-2" />
								</div>
						...
						<tr :key="'adress-eng' + index">
							<td colspan="2">
								<div class="flex items-center text-sm">
									<label class="w-48 px-3 py-2 text-center border-2 bg-gray-200">{{ $t("project.create_page.address.subtitle_lang.en") }}</label>
									<input v-model="form.address_en[index]" type="text" class="w-full px-3 py-2 text-center border-2" />
								</div>
			...
			<div class="flex items-center gap-2 text-white justify-center py-3">
				<button @click="addLocation()" class="w-1/3 py-3 text-center bg-green-500 rounded shadow-sm cursor-pointer">{{ $t("button.add") }}</button>
			</div>
		</div>
	</div>
</template>
...
export default {
	...
	mounted() {
		...
		const L = window.L;
		this.map = L.map("mapEdit-" + this.form._id, { zoomControl: false });
	...
	methods: {
		...
		addLocation() {
			...
			if(this.form.location.features.length === this.form.address.length + 1){
				this.form.address.push("");
			}
			if(this.form.location.features.length === this.form.address_en.length + 1){
