<template>
	<div class="w-full grid grid-cols-1 xl:grid-cols-1 overflow-x-auto shadow-sm rounded">
		<table class="items-center w-full space-y-6">
			<thead>
				<tr class="bg-tgo-teal-500 text-sm font-medium text-gray-200">
					<ProjectTHSlot v-for="(header, idx) in table.head" :key="idx" :header="header" @setSortValue="setSortValue(header.sortKey, $event)" />
				</tr>
			</thead>
			<tbody class="bg-white">
				<template v-for="(data, index) in project">
					<tr tabindex="0" :class="{ 'bg-gray-100': index % 2 == 1 }" class="focus:outline-none text-xs h-10 rounded hover:bg-tgo-yellow-100 align-middle cursor-pointer" @click="Onpage(data._id)" :key="index">
						...
						<td class="border-t-0 px-1 align-center whitespace-nowrap justify-center text-center">
							<!-- <div class="w-64 h-auto break-all relative flex justify-center"> -->
							<p class="font-medium text-xs text-theme-black-300 leading-6 ml-2 truncate">{{ data.standard || "Standard T-VER" }}</p>
							<!-- </div> -->
						</td>
						<td class="border-t-0 px-1 align-center whitespace-nowrap justify-center text-center">
							<span class="font-medium text-xs text-theme-black-300 leading-6 ml-2">{{ dateLocale(data.registration_date) }}</span>
						</td>
						<td class="border-t-0 px-1 align-center whitespace-nowrap justify-center text-center">
							<span class="font-medium text-xs text-theme-black-300 leading-6 ml-2">{{ dateLocale(data.valid_start) }} - {{ dateLocale(data.valid_end) }}</span>
						</td>
