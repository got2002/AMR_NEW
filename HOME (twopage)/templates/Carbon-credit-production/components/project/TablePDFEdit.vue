	...
	<div class="w-full grid grid-cols-1 xl:grid-cols-1 2xl:grid-cols-1 mt-2">
		<table class="items-center w-full space-y-6">
			...
			<tbody class="bg-white border border-theme-black-300">
				<template v-for="(data, index) in form.registration_files">
					<tr :key="index" tabindex="0" class="focus:outline-none text-xs h-12 rounded hover:bg-theme-green-100 align-middle border border-theme-black-300">
						<td class="border-t-0 px-1 align-center whitespace-nowrap justify-center text-center border border-theme-black-300">
							<div class="w-full flex items-center">
								...
								<span class="font-medium text-sm text-theme-black-300 leading-6 ml-2 truncate w-28 md:w-32 lg:w-48 xl:w-72 2xl:w-96">{{ data.name }}</span>
							</div>
						</td>
						<td class="border-t-0 px-1 align-center whitespace-nowrap justify-center text-center border border-theme-black-300">
		...
		<div v-if="form.registration_files == 0" tabindex="0" colspan="9" class="outline-none text-xs h-16 bg-white rounded-b  align-middle border border-t-0 border-gray-400">
...
export default {
	...
	methods: {
		uploadSuccess(file, response) {
			this.form.registration_files.push(response);
		...
		deletePDF(index) {
			let num = this.form.registration_files.length - this.fileArr.length;
			if (index - num >= 0) {
				this.fileArr.splice(index - num, 1);
			}
			this.form.registration_files.splice(index, 1);
		},
		...
		removeImg(idx) {
			...
			this.form.registration_files.splice(idx + num, 1);
		},
		dateToString(date) {
