<template>
	<div class="w-full grid grid-cols-1 xl:grid-cols-1 2xl:grid-cols-1 mt-2">
					...
					<tr :key="index" class="text-xs h-12 hover:bg-theme-green-100 align-middle divide-y divide-x divide-black">
						<td class="px-1 border-t border-black">
							<div class="w-full flex items-center">
								...
								<span :title="data.name" class="font-medium text-sm text-theme-black-300 leading-6 ml-2 truncate w-28 md:w-32 lg:w-48 xl:w-48 2xl:w-96">{{ data.name }}</span>
							</div>
						</td>
		...
		<div v-if="form.registration_files == 0" class="outline-none text-xs h-16 bg-white rounded-b  align-middle border border-t-0 border-gray-400">
