<template>
	<div class="fixed z-50 top-0 left-0 w-full h-full bg-black bg-opacity-60 flex items-center justify-center">
		<div data-aos="fade-down" data-aos-anchor-placement="top-center" class="mx-auto container bg-white rounded shadow-sm p-4 w-4/12 space-y-4">
	...
	export default {
		...
		methods: {
			...
			getToken() {
				...
				this.loading = true;
				this.$axios
					.$get(`/api/v1/account/sync/${this.account_id}/request`)
					.then((resp) => {
						app.loading = false;
