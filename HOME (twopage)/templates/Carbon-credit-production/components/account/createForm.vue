<template>
	<div class="flex flex-col h-full">
		<div class="mb-3">
			<div class="bg-white rounded border-b pb-2">
				<h2 class="font-semibold">{{ $t("account.create_page.form.title") }}</h2>
			</div>
			<div id="account__form" class="grid grid-cols-1 gap-2 mt-3">
				<div>
					<UILabel :text="$t('account.create_page.form.name')" :required="true" />
					<UITextInput v-model="form.accountName" />
					...
					<UIErrorMsg v-if="!$v.form.accountName.required && isSubmitted">{{ $t("form_validation.require.Accountname") }}</UIErrorMsg>
					<UIErrorMsg v-if="!$v.form.accountName.containsNotSpecial && isSubmitted">{{ $t("form_validation.pattern.Accountname") }}</UIErrorMsg>
					<UIErrorMsg v-if="!$v.form.accountName.minLength && isSubmitted">{{ $t("form_validation.min_length.Accountname") }}</UIErrorMsg>
				</div>
				<div>
					<UILabel :text="$t('account.create_page.form.email')" :required="true" />
					<UIDropdown v-model="form.userEmail" :option="dropdowns.email" />
					<!-- <label class="text-gray-600 font-semibold text-sm mb-2 ml-1">{{ $t("account.create_page.form.email") }} <span class="text-red-600">*</span></label>
					<div>
						
						<v-select :options="dropdowns.email" :reduce="(text) => text.value" label="text" v-model="form.userEmail" />
					</div> -->
					<UIErrorMsg v-if="!$v.form.userEmail.selectDropdown && isSubmitted">{{ $t("form_validation.require.email") }}</UIErrorMsg>
				</div>
				<div>
					<UILabel :text="$t('account.create_page.form.accountTypes')" :required="true" />
					<UIDropdown v-model="form.accountTypeID" :option="dropdowns.transaction_types" />
					...
					<UIErrorMsg v-if="!$v.form.accountTypeID.selectDropdown && isSubmitted">{{ $t("form_validation.require.accountTypes") }}</UIErrorMsg>
				</div>
			</div>
		</div>
	</div>
