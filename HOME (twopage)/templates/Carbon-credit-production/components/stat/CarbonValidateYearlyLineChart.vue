<template>
	<div class="h-full w-full p-4 space-y-4">
		<ChartLine ref="validateYearly" :data="ChartData" :option="ChartOptions" :height="300"></ChartLine>
	</div>
</template>
...
export default {
	...
	methods: {
		createXlabels() {
			...
			this.$refs.validateYearly._data._chart.update()
		...
		getBackgroundGradient(context) {
			...
			if (!gradient || width !== chartWidth || height !== chartHeight) {
				...
				gradient.addColorStop(1, "#2061a260");
				// gradient.addColorStop(1, "#a7c4e1");
			}
		...
		async getVerificatedCreditStat() {
			...
			return await app.$axios
				.$get(`/api/v1/stats/statistics/approx-carbon-reduction-years`)
				.then((resp) => {
					...
					app.$refs.validateYearly._data._chart.update();
				})
				.catch((err) => err);
		},
	},
