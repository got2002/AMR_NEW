<template>
	<div class="h-full w-full p-4 space-y-4">
		<ChartBar ref="projectYearly" :data="ChartData" :option="ChartOptions" :height="300"></ChartBar>
	</div>
</template>
...
export default {
	...
	methods: {
		createXlabels() {
			...
			this.$refs.projectYearly._data._chart.update()
		...
		async getVerificatedCreditStat() {
			...
			return await app.$axios
				.$get(`/api/v1/stats/statistics/project-classified-years`)
				.then((resp) => {
					...
					app.$refs.projectYearly._data._chart.update();
				})
				.catch((err) => err);
		},
	},
