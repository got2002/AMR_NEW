<template>
	<div class="fixed z-50 top-0 left-0 bg-black bg-opacity-60 w-full h-full flex items-center justify-center overflow-hidden">
		<div data-aos="fade-down" data-aos-anchor-placement="top-center" class="2xl:w-3/12 lg:w-6/12 md:w-8/12 w-10/12 bg-white p-10 rounded shadow-sm">
			<p class="text-2xl font-bold">{{ $t("buffer.modal.title2") }}</p>
			...
			<div id="transfer" class="grid grid-cols-2 gap-4 text-sm">
				<div class="col-span-2">
					...
					<p class="text-xs text-red-500" v-if="!$v.percentBuffer.min && isSubmitted">{{ $t("buffer.modal.validation.required") }}</p>
					<div class="w-full flex items-center justify-between mt-2 text-sm">
						<div class="text-sm text-gray-500">{{ $t(`buffer.modal.reversal`) }} (tCO<sub>2</sub>eq)</div>
			...
			app.$swal
				.fire({
					...
					iconHtml: `
								...
								`,
					title: this.$t("sweetalert.buffer.confirm.title2"),
					text: this.$t("sweetalert.buffer.confirm.sub_title2"),
				...
				.then((result) => {
					if (result.isConfirmed) {
						...
						app.$axios
							.$post(`/api/v1/buffercarbons/reversal_retirement/${app.info._id}`, {
							...
							.then((resp) => {
								...
								app.$swal
									.fire({
										...
										title: app.$t("sweetalert.buffer.success.title2"),
							...
							.catch((err) => {
								...
								app.$swal.fire({
									icon: "error",
									title: app.$t("sweetalert.buffer.fail.title2"),
