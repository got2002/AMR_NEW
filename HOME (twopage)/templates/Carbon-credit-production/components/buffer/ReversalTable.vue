			...
			<tbody class="bg-white">
				<template v-for="(data, index) in project">
					<tr :key="index" :class="{ 'bg-gray-100 hover:bg-gray-200': index % 2 == 1 }" class="focus:outline-none text-xs hover:bg-gray-50 h-10 rounded align-middle">
						...
						<CreditDataSlot :text="dateLocale(data.created_at)" align="center"></CreditDataSlot>
					</tr>
				</template>
			</tbody>
		</table>
		<BufferModal v-if="modalOpen" @close="modalOpen = false" @submit="reloadData" :accountNumber="accountNumber" :info="selectInfo"></BufferModal>
		...
		return {
			...
			table: {
				head: [
					...
					{
						name: this.$t("buffer.table.createdAt"),
						align: "center",
						sortable: true,
						sortKey: "createdAt",
						sortValue: 0,
