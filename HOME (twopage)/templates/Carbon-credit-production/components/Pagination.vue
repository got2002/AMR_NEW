<template>
	<div v-if="totalPages > 0" class="flex m-2 px-0 text-xs">
		<!-- <slot :start="start + 1" :end="end" :total="totalItems">
			<div class="pr-2 leading-loose">{{ start + 1 }} - {{ end }} of {{ totalItems }} items</div>
		</slot> -->
		<div v-if="totalPages > 1" class="flex-grow flex justify-end">
			<slot :buttons="buttons" name="buttons">
				<vue-ads-page-button v-for="(button, key) in buttons" :key="key" v-bind="button" @page-change="$emit('page-change', button.page)" />
			</slot>
		</div>
	</div>
</template>
...
export default {
	...
	computed: {
		...
		pages() {
			...
			let pages = filteredPages ? [filteredPages[0] - 1 === 1 ? 1 : "...", ...filteredPages, filteredPages[filteredPages.length - 1] + 1 === this.totalPages - 2 ? this.totalPages - 2 : "..."] : [...Array(this.totalPages - 2).keys()].map((page) => page + 1);
