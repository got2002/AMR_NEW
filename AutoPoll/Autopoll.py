...
def convert_raw_to_value(data_type, raw_data):
    if data_type == "Date":
        ...
        rounded_float_value = round(float_value, 5)  # ปรับตำแหน่งทศนิยมเป็น 5 ตำแหน่ง
...
for row in rows:
    ...
    sql_text_config_insert+=f"TO_DATE('{date_system}', 'DD-MM-YYYY'), '{METERID}','{run}','{evc_type}',TO_TIMESTAMP('{current_datetime}', 'DD-MM-YYYY HH24:MI:SS.FF') AT TIME ZONE 'ASIA/BANGKOK',"
    ...
    for i in range(0, len(df_mappingbilling), 5):
            ...
            sql_text_billing_insert += f"TO_TIMESTAMP('{current_datetime}', 'DD-MM-YYYY HH24:MI:SS.FF') AT TIME ZONE 'ASIA/BANGKOK'"
