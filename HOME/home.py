...
WHERE
 ...
 # Get selected values from the dropdowns
    ...
    if selected_region:
        ...
        # ส่ง DataFrame ไปยัง HTML template
        return render_template('Daily_summary.html', tables=[df.to_html(classes='data',index=False)],
                            titles=df.columns.values,
                            selected_region=selected_region,
                            region_options=region_options)
    else:
        # Render the template without executing the query
        return render_template('Daily_summary.html', selected_region=selected_region, region_options=region_options, tables=[])
############ /Daily summary  #####################
